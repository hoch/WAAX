...
UI.IndexedListController.prototype = {
  ...
  _updateIndexFromValue: function (value) {
    ...
    // if value can't be found, do nothing return not changed
    if (idx === -1) {
      return false;
    }
    // if new and prev values are same, do nothing return false (not changed)
    if (this._value === this._data[idx]) {
      return false;
    } else {
      this._index = idx;
  ...
  _updateValueFromIndex: function (index) {
    var idx = UI.clamp(index, 0, this._data.length - 1);
    // if new and prev values are same, do nothing return false (not changed)
    if (this._value === this._data[idx]) {
      return false;
    } else {
  ...
  setValue: function (value, flag) {
    // if value is empty string or the same as previous, ignore.
    if (value === "") {
      return;
    }
    // value is (-1 || 1 || string)
    var changed = true;
    if (value === -1 || value === 1) {
      changed = this._updateValueFromIndex(this._index + value);
    } else {
      changed = this._updateIndexFromValue(value);
    }
  ...
  */
  ...
  setParams: function (params) {
    ...
    if (params.data.length === 0) {
      console.log(this._name + " : empty data.");
    }
    this._index = 0;
    this._data = params.data;
    // prepare value(string) and index(number)
    this._updateIndexFromValue(params.value || params.data[0]);
