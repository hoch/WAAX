...
(function (WX) {
  ...
  var _LOG2 = Math.LN2,
      _LOG10 = Math.LN10,
      _MIDI1499 = 3.282417553401589e+38;
  ...
  WX.mtof = function (midi) {
    ...
    else if (midi > 1499) return _MIDI1499;
  ...
  WX.ftom = function (freq) {
    return Math.floor(
      freq > 0 ?
      Math.log(freq/440.0) / _LOG2 * 12 + 69 : -1500
    );
  };
  ...
  WX.powtodb = function (power) {
    if (power <= 0) return 0;
      ...
      var db = 100 + 10.0 / _LOG10 * Math.log(power);
      return db < 0 ? 0 : db;
    }
  };
  ...
  WX.dbtopow = function (db) {
    if (db <= 0) return 0;
      ...
      // TODO: what is 870?
      if (db > 870) db = 870;
      return Math.exp(_LOG10 * 0.1 * (db - 100.0));
    }
  };
  ...
  WX.rmstodb = function (rms) {
    if (rms <= 0) return 0;
      ...
      var db = 100 + 20.0 / _LOG10 * Math.log(rms);
      return db < 0 ? 0 : db;
    }
  };
  ...
  WX.dbtorms = function (db) {
    if (db <= 0) return 0;
      ...
      // TODO: what is 485?
      if (db > 485) db = 485;
      return Math.exp(_LOG10 * 0.05 * (db - 100.0));
    }
  };
  ...
  WX.lintodb = function (lin) {
    ...
    return 20.0 * (lin > 0.00001 ? (Math.log(lin) / _LOG10) : -5.0);
  };
